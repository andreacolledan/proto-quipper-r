...
synthesizeType :: Expr -> StateT TypingEnvironment (Either TypeError) (Type, Index)
-- UNIT
synthesizeType EUnit = withScope EUnit $ return (TUnit, Number 0)
-- LABEL
synthesizeType e@(ELabel id) = withScope e $ do
  btype <- labelContextLookup id
  return (TWire btype, Number 1)
-- VARIABLE
synthesizeType e@(EVar id) = withScope e $ do
  typ <- typingContextLookup id
  return (typ, wireCount typ)
-- PAIR
synthesizeType e@(EPair e1 e2) = withScope e $ do
  (typ1, i1) <- synthesizeType e1
  ((typ2, i2), wc) <- withWireCount $ synthesizeType e2
...
-- NIL
synthesizeType e@(ENil anno) = withScope e $ case anno of
...
-- ABSTRACTION
synthesizeType e@(EAbs x annotyp e1) = withScope e $ do
  checkWellFormedness annotyp
  ((typ, i), wc) <- withWireCount $ withBoundVariable x annotyp $ synthesizeType e1
  return (TArrow annotyp typ i wc, wc)
-- LIFT
synthesizeType e@(ELift e1) = withScope e $ do
  (typ, i) <- withNonLinearContext $ synthesizeType e1
  unless (checkEq i (Number 0)) $ throwLocalError $ UnexpectedWidthAnnotation e1 (Number 0) i
  return (TBang typ, Number 0)
-- CONS
synthesizeType e@(ECons e1 e2) = withScope e $ do
  (typ1, i1) <- synthesizeType e1
  ((TList j typ1', i2), wc) <- withWireCount $ synthesizeType e2
  unless (checkSubtype (TList j typ1') (TList j typ1)) $ throwLocalError $ UnexpectedType e2 (TList j typ1) (TList j typ1')
...
-- FOLD
synthesizeType e@(EFold e1 e2 e3) = withScope e $ do
  -- naming conventions are not satisfied here because the rule is HARD to parse
  (steptyp@(TBang (TIForall id (TArrow (TPair acctyp elemtyp) acctyp' stepwidth o1) o2 o3)), i1) <- synthesizeType e1
  unless (checkEq o1 (Number 0)) $ throwLocalError $ UnexpectedIndex (Number 0) o1
  unless (checkEq o2 (Number 0)) $ throwLocalError $ UnexpectedIndex (Number 0) o2
  unless (checkEq o3 (Number 0)) $ throwLocalError $ UnexpectedIndex (Number 0) o3
  ...
  ((acctyp'', i2), wc1) <- withWireCount $ synthesizeType e2
  unless (checkSubtype acctyp'' (isub (Number 0) id acctyp)) $ throwLocalError $ UnfoldableAccumulator e2 acctyp''
  ((argtyp@(TList arglen elemtyp'), i3), wc2) <- withWireCount $ synthesizeType e3
...
-- BOXED CIRCUIT
synthesizeType e@(ECirc b1 c b2) = withScope e $ do
...
synthesizeType e@(EApp e1 e2) = withScope e $ do
  (TArrow annotyp typ3 j1 j2, i1) <- synthesizeType e1
  ((typ2, i2), wc) <- withWireCount $ synthesizeType e2
  ...
  return (typ3, k)
-- APPLY (CIRCUITS)
synthesizeType e@(EApply e1 e2) = withScope e $ do
  (TCirc j inbt outbt, i1) <- synthesizeType e1
  ((typ2, i2), wc) <- withWireCount $ synthesizeType e2
  ...
  return (outtyp, k)
-- BOX
synthesizeType e@(EBox bt e1) = withScope e $ do
  (typ1@(TBang (TArrow typ2 typ3 j1 _)), i) <- synthesizeType e1
...
-- LET-IN
synthesizeType e@(ELet x e1 e2) = withScope e $ do
  (typ1, i1) <- synthesizeType e1
  ((typ2, i2), wc) <- withWireCount $ withBoundVariable x typ1 $ synthesizeType e2
  -- max(i1 + wires in e2, i2):
  let k = Max (Plus i1 wc) i2
  return (typ2, k)
-- DESTRUCTURING LET-IN
synthesizeType e@(EDest x y e1 e2) = withScope e $ do
  (TPair typ2 typ3, i1) <- synthesizeType e1
  ((typ4, i2), wc) <- withWireCount $ withBoundVariable x typ2 $ withBoundVariable y typ3 $ synthesizeType e2
  let k = Max (Plus i1 wc) i2
  return (typ4, k)
-- ANNOTATION
synthesizeType e@(EAnno e1 annotyp) = withScope e $ do
  checkWellFormedness annotyp
  (typ, i) <- synthesizeType e1
  unless (checkSubtype typ annotyp) $ throwLocalError $ UnexpectedType e1 annotyp typ
  return (annotyp, i)
-- FORCE
synthesizeType e@(EForce e1) = withScope e $ do
  (TBang typ', i) <- synthesizeType e1
  return (typ', i)
-- INDEX ABSTRACTION
synthesizeType e@(EIAbs id e1) = withScope e $ do
  ((typ, i), wc) <- withWireCount $ withBoundIndexVariable id $ synthesizeType e1
  return (TIForall id typ i wc, i)
-- INDEX APPLICATION
synthesizeType e@(EIApp e1 g) = withScope e $ do
  (TIForall id typ2 j1 _, i) <- synthesizeType e1
  checkWellFormedness j1
  return (isub g id typ2, Max i (isub g id j1))
-- CONSTANTS
synthesizeType e@(EConst c) = withScope e $ return (typeOf c, Number 0)
-- LET-CONS
synthesizeType e@(ELetCons x y e1 e2) = withScope e $ do
  (typ1@(TList j typ2), i1) <- synthesizeType e1
  unless (checkLeq (Number 1) j) $ throwLocalError $ UnexpectedEmptyList e1 typ1
  ((typ3, i2), wc) <- withWireCount $ withBoundVariable x typ2 $ withBoundVariable y (TList (Minus j (Number 1)) typ2) $ synthesizeType e2
  let k = Max (Plus i1 wc) i2
  return (typ3, k)

--- EXPORTED WRAPPER FUNCTIONS ---------------------------------------------------------------

runTypeInferenceWith :: TypingEnvironment -> Expr -> Either TypeError (Type, Index)
runTypeInferenceWith env e = do
  e' <- annotateNil env e
  ((typ, i), remaining) <- runStateT (synthesizeType e') env
...
runTypeInference :: Expr -> Either TypeError (Type, Index)
