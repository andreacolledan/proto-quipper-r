...
import AST.Bundle (WireType (Bit, Qubit))
import qualified AST.Bundle as Bundle
import AST.Circuit
import AST.Index
import AST.Language (Term (..), Type (..), Value (..))
...
import Semantics.Type (checkTypeEq, simplifyType)
import Test.Hspec
      ...
      it "succeeds when the rule's premises hold" $ do
        ...
        let term =
              App
                ( Abs
                    "p"
                    (Tensor (WireType Qubit) (WireType Bit))
