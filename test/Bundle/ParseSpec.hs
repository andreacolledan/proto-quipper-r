...
spec :: Spec
spec = describe "bundle parser" $ do
  it "parses a label" $ do
    parse parseBundle "" "a" `shouldBe` Right (Label "a")
  it "parses a tuple" $ do
    parse parseBundle "" "(a, b, c)" `shouldBe` Right (Tuple [Label "a", Label "b", Label "c"])
  it "parses a unit" $ do
  ...
  it "parses complex expressions" $ do
    parse parseBundle "" "([a, b], c)" `shouldBe` Right (Tuple [Cons (Label "a") (Cons (Label "b") Nil), Label "c"])
