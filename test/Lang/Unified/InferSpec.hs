    ...
    -- Tests for the type inference of terms that do not produce any side-effect
      context "when typing the unit value" $ do
        it "produces the unit type if the context is empty" $ \qfh -> do
          -- ∅;∅;∅ ⊢ () ==> () ; 0
          runTypeInferenceWith emptyEnv EUnit qfh `shouldBe` Right (TUnit, Number 0)
        it "produces the unit type if the context is non-linear" $ \qfh -> do
          ...
          runTypeInferenceWith (makeEnv gamma []) EUnit qfh `shouldBe` Right (TUnit, Number 0)
        it "fails when there are linear resources in the environment" $ \qfh -> do
          -- ∅;x:Qubit;∅ ⊢ () =/=>
          let gamma = [("x", TWire Qubit)]
          runTypeInferenceWith (makeEnv gamma []) EUnit qfh `shouldSatisfy` isLeft
          -- ∅;∅;x:Qubit ⊢ () =/=>
          let q = [("x", Qubit)]
          runTypeInferenceWith (makeEnv [] q) EUnit qfh `shouldSatisfy` isLeft
      context "when typing labels" $ do
        it "produces the type of the label if the rest of the context is empty" $ \qfh -> do
          -- ∅;∅;l:Qubit ⊢ l ==> Qubit ; 1
          let q = [("l", Qubit)]
          runTypeInferenceWith (makeEnv [] q) (ELabel "l") qfh `shouldBe` Right (TWire Qubit, Number 1)
          -- ∅;∅;l:Bit ⊢ l ==> Bit ; 1
          let b = [("l", Bit)]
          runTypeInferenceWith (makeEnv [] b) (ELabel "l") qfh `shouldBe` Right (TWire Bit, Number 1)
        it "produces the type of the label if the rest of the context is nonlinear" $ \qfh -> do
          -- ∅;x:();l:Qubit ⊢ l ==> Qubit ; 1
          let gamma = [("x", TUnit)]
          let q = [("l", Qubit)]
          runTypeInferenceWith (makeEnv gamma q) (ELabel "l") qfh `shouldBe` Right (TWire Qubit, Number 1)
        it "fails when there are other linear resources in the environment" $ \qfh -> do
          -- ∅;x:Qubit;l:Qubit ⊢ l =/=>
          let gamma = [("x", TWire Qubit)]
          let q = [("l", Qubit)]
          runTypeInferenceWith (makeEnv gamma q) (ELabel "l") qfh `shouldSatisfy` isLeft
          -- ∅;∅;x:Qubit,l:Qubit ⊢ l =/=>
          let q = [("x", Qubit), ("l", Qubit)]
          runTypeInferenceWith (makeEnv [] q) (ELabel "l") qfh `shouldSatisfy` isLeft
      context "when typing variables" $ do
        it "produces the type of the variable if the rest of the context is empty" $ \qfh -> do
          -- ∅;x:Qubit;∅ ⊢ x ==> Qubit ; 1
          let gamma = [("x", TWire Qubit)]
          runTypeInferenceWith (makeEnv gamma []) (EVar "x") qfh `shouldBe` Right (TWire Qubit, Number 1)
        it "produces the type of the variable if the rest of the context is nonlinear" $ \qfh -> do
          -- ∅;x:(),y:Qubit;∅ ⊢ y ==> Qubit ; 1
          let gamma = [("x", TUnit), ("y", TWire Qubit)]
          runTypeInferenceWith (makeEnv gamma []) (EVar "y") qfh `shouldBe` Right (TWire Qubit, Number 1)
        it "fails when there are other linear resources in the environment" $ \qfh -> do
          -- ∅;x:Qubit,y:Qubit;∅ ⊢ y =/=>
          let gamma = [("x", TWire Qubit), ("y", TWire Qubit)]
          runTypeInferenceWith (makeEnv gamma []) (EVar "y") qfh `shouldSatisfy` isLeft
      ...
      context "when typing the empty list" $ do
        it "produces an empty list type and wirecount zero if the context is empty" $ \qfh -> do
          -- ∅;∅;∅ ⊢ [] :: List[0] () ==> List[0] () ; 0
          runTypeInferenceWith emptyEnv (EAnno (ENil Nothing) (TList (Number 0) TUnit)) qfh `shouldBe` Right (TList (Number 0) TUnit, Number 0)
        it "produces an empty list type and wirecount zero if the context is non-linear" $ \qfh -> do
          -- ∅;x:();∅ ⊢ [] :: List[0] () ==> List[0] () ; 0
          let gamma = [("x", TUnit)]
          runTypeInferenceWith (makeEnv gamma []) (EAnno (ENil Nothing) (TList (Number 0) TUnit)) qfh `shouldBe` Right (TList (Number 0) TUnit, Number 0)
        it "fails when there are linear resources in the environment" $ \qfh -> do
          -- ∅;x:Qubit;∅ ⊢ [] :: List[0] () =/=> 
          let gamma = [("x", TWire Qubit)]
          runTypeInferenceWith (makeEnv gamma []) (EAnno (ENil Nothing) (TList (Number 0) TUnit)) qfh `shouldSatisfy` isLeft
          -- ∅;∅;l:Qubit ⊢ [] :: List[0] () =/=>
          let q = [("l", Qubit)]
          runTypeInferenceWith (makeEnv [] q) (EAnno (ENil Nothing) (TList (Number 0) TUnit)) qfh `shouldSatisfy` isLeft
        it "fails when the parameter type is unconstrained" $ \qfh -> do
          -- ∅;∅;∅ ⊢ [] =/=>
          runTypeInferenceWith emptyEnv (ENil Nothing) qfh `shouldSatisfy` isLeft
      context "when typing cons" $ do
        ...
        it "fails when the head is of a different type than the rest of the list" $ \qfh -> do
          ...
          let gamma = [("x", TWire Qubit), ("y", TWire Bit)]
          let expr = ECons (EVar "x") (ECons (EVar "y") (ENil Nothing))
          runTypeInferenceWith (makeEnv gamma []) expr qfh `shouldSatisfy` isLeft
      context "when typing abstractions" $ do
        ...
        it "fails when the body uses the argument more than once" $ \qfh -> do
          ...
          runTypeInferenceWith emptyEnv expr qfh `shouldSatisfy` isLeft
      ...
      context "when typing index abstraction" $ do
        ...
        it "fails if the index variable already exists" $ \qfh -> do
          -- i;∅;∅ ⊢ @i . () =/=>
          let expr = EIAbs "i" EUnit
          runTypeInferenceWith (makeEnvForall ["i"] [] []) expr qfh `shouldSatisfy` isLeft
          -- ∅;∅;∅ ⊢ @i . (@i . ()) @i =/=>
    ...
    -- Either by nature or because some sub-terms are effectful
      ...
      context "when typing application" $ do
        ...
        it "fails if the function is not a function type" $ \qfh -> do
          ...
          runTypeInferenceWith (makeEnv [] q) expr qfh `shouldSatisfy` isLeft
