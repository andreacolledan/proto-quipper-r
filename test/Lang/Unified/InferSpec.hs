  ...
  describe "type inference on values" $ do
    context "when typing the unit value" $ do
      ...
      it "produces the unit type if the context is non-linear" $ do
        ...
        let gamma = Map.fromList [("x", TUnit), ("y", TCirc (Number 1) (BTWire Qubit) (BTWire Qubit))]
        runTypeInferenceWith (makeEnv gamma emptyctx) EUnit `shouldBe` Right (TUnit, Number 0)
      it "fails when there are linear resources in the environment" $ do
        -- ∅;x:Qubit;∅ ⊢ () =/=>
        let gamma = Map.fromList [("x", TWire Qubit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) EUnit `shouldSatisfy` isLeft
        -- ∅;∅;x:Qubit ⊢ () =/=>
        let q = Map.fromList [("x", Qubit)]
        runTypeInferenceWith (makeEnv emptyctx q) EUnit `shouldSatisfy` isLeft
    context "when typing labels" $ do
      it "produces the type of the label if the rest of the context is empty" $ do
        ...
        runTypeInferenceWith (makeEnv emptyctx q) (ELabel "l") `shouldBe` Right (TWire Qubit, Number 1)
        -- ∅;∅;l:Bit ⊢ l ==> Bit ; 1
        let b = Map.fromList [("l", Bit)]
        runTypeInferenceWith (makeEnv emptyctx b) (ELabel "l") `shouldBe` Right (TWire Bit, Number 1)
      it "produces the type of the label if the rest of the context is nonlinear" $ do
        -- ∅;x:();l:Qubit ⊢ l ==> Qubit ; 1
        let gamma = Map.fromList [("x", TUnit)]
        let q = Map.fromList [("l", Qubit)]
      ...
      it "fails when there are other linear resources in the environment" $ do
        ...
        -- ∅;∅;x:Qubit,l:Qubit ⊢ l =/=>
        let q = Map.fromList [("x", Qubit), ("l", Qubit)]
        runTypeInferenceWith (makeEnv emptyctx q) (ELabel "l") `shouldSatisfy` isLeft
    context "when typing variables" $ do
      it "produces the type of the variable if the rest of the context is empty" $ do
        -- ∅;x:Qubit;∅ ⊢ x ==> Qubit ; 1
        let gamma = Map.fromList [("x", TWire Qubit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) (EVar "x") `shouldBe` Right (TWire Qubit, Number 1)
      it "produces the type of the variable if the rest of the context is nonlinear" $ do
        -- ∅;x:(),y:Qubit;∅ ⊢ y ==> Qubit ; 1
        let gamma = Map.fromList [("x", TUnit), ("y", TWire Qubit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) (EVar "y") `shouldBe` Right (TWire Qubit, Number 1)
      it "fails when there are other linear resources in the environment" $ do
        -- ∅;x:Qubit,y:Qubit;∅ ⊢ y =/=>
        let gamma = Map.fromList [("x", TWire Qubit), ("y", TWire Qubit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) (EVar "y") `shouldSatisfy` isLeft
    context "when typing pairs" $ do
      context "when the subexpressions are values" $ do
        it "produces the correct tensor type and the sum of the wirecounts of the elements" $ do
          ...
          let gamma = Map.fromList [("x", TWire Qubit), ("y", TWire Bit)]
          let expr = EPair (EVar "x") (EVar "y")
          let expected = (TPair (TWire Qubit) (TWire Bit), Number 2)
          simplify (runTypeInferenceWith (makeEnv gamma emptyctx) expr) `shouldBe` Right expected
          -- ∅;x:Qubit,y:Bit,z:Qubit;∅ ⊢ (x,(y,z)) ==> (Qubit,(Bit,Qubit)) ; 3
          let gamma = Map.fromList [("x", TWire Qubit), ("y", TWire Bit), ("z", TWire Qubit)]
    ...
    context "when typing the empty list" $ do
      ...
      it "produces an empty list type and wirecount zero if the context is non-linear" $ do
        -- ∅;x:();∅ ⊢ [] ==> List[0] a0 ; 0
        let gamma = Map.fromList [("x", TUnit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) ENil `shouldBe` Right (TList (Number 0) (TVar "a0"), Number 0)
      it "fails when there are linear resources in the environment" $ do
        -- ∅;x:Qubit;∅ ⊢ [] =/=> 
        let gamma = Map.fromList [("x", TWire Qubit)]
        runTypeInferenceWith (makeEnv gamma emptyctx) ENil `shouldSatisfy` isLeft
        -- ∅;∅;l:Qubit ⊢ [] =/=>
        let q = Map.fromList [("l", Qubit)]
        runTypeInferenceWith (makeEnv emptyctx q) ENil `shouldSatisfy` isLeft
    context "when typing cons" $ do
      it "produces the correct list type and the wirecount of the elements times the length of the list" $ do
        ...
        let gamma = Map.fromList [("x", TPair (TWire Bit) (TWire Bit)), ("xs", TList (Number 100) (TPair (TWire Bit) (TWire Bit)))]
      ...
      it "fails when the head is of a different type than the rest of the list" $ do
        ...
        let gamma = Map.fromList [("x", TWire Qubit), ("y", TWire Bit)]
        let expr = ECons (EVar "x") (ECons (EVar "y") ENil)
        runTypeInferenceWith (makeEnv gamma emptyctx) expr `shouldSatisfy` isLeft
    context "when typing abstractions" $ do
      it "produces the correct arrow type and a wirecount equal to the second annotation of the arrow" $ do
        ...
        let theta = Set.singleton "i"
        simplify (runTypeInferenceWith (makeEnvForall theta gamma emptyctx) expr) `shouldBe` Right expected
      ...
      it "fails when the body does not use the argument" $ do
        ...
        runTypeInferenceWith emptyEnv expr `shouldSatisfy` isLeft
      it "fails when the formal parameter shadows an existing linear variable" $ do
        -- ∅;x:Qubit;∅ ⊢ (\x :: Qubit . x) x =/=>
        let gamma = Map.fromList [("x", TWire Qubit)]
    ...
    context "when typing index abstraction" $ do
      ...
      it "fails if the index variable already exists" $ do
        -- i;∅;∅ ⊢ @i . () =/=>
        let expr = EIAbs "i" EUnit
        runTypeInferenceWith (makeEnvForall (Set.singleton "i") emptyctx emptyctx) expr `shouldSatisfy` isLeft
        -- ∅;∅;∅ ⊢ @i . (@i . ()) @i =/=>
